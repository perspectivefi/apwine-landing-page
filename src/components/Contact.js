            ...
            render={({ subscribe, status, message }) => <>
                <form className="flex flex-col sm:flex-row justify-center px-8" onSubmit={e => { subscribe({ EMAIL: email }); e.preventDefault() }}>
                    <input type="email" required placeholder="Enter your email" onChange={e => setEmail(e.target.value)}
                        class="block sm:max-w-xs w-full px-4 py-3 text-base leading-6 appearance-none border border-gray-300 shadow-none bg-white rounded-md placeholder-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300" />
                    <button class="mt-4 relative sm:mt-0 sm:h-auto sm:ml-4 block w-full sm:w-auto border border-transparent px-6 py-3 text-base leading-6 font-semibold leading-snug bg-gray-900 text-white rounded-md shadow-md hover:bg-gray-800 focus:outline-none focus:bg-gray-800 transition ease-in-out duration-150 hover:bg-gray-600" type="sub">
                        <span class={status === "sending" ? "opacity-0" : ""}>Sign up</span>
                        <span class={classnames("absolute inset-0 flex items-center justify-center pointer-events-none", status === "sending" ? "opacity-100" : "opacity-0")}>
                            <svg class="h-8 w-8 spin" style={{ animation: "spin 1s infinite linear" }} viewBox="0 0 24 24">
...
const Contact = () => (
    <section class="py-32 bg-blue-500">
        <div class="text-center text-white px-3 lg:px-0">
            <h1 class="my-4 text-2xl md:text-3xl lg:text-5xl font-black leading-tight">
                Interested?
            </h1>
            <p class="leading-loose text-gray-900 text-base md:text-lg lg:text-xl mb-8">
